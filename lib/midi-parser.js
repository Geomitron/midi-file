...
Parser.prototype.readString = function(len) {
  // MIDI text meta events (text, trackName, lyrics, marker, cuePoint, …) have
  // no formal encoding in the SMF spec. Most files use either Latin-1 (one
  // codepoint per byte) or UTF-8 (variable-length multibyte). Decode both
  // ways and pick the one whose resulting codepoint count differs from the
  // byte count — that's the signal that the source was UTF-8.
  var bytes = this.readBytes(len)
  var singlebyteString = String.fromCharCode.apply(null, bytes)
  var multibyteString = new TextDecoder().decode(bytes instanceof Uint8Array ? bytes : new Uint8Array(bytes))
  // If decoding as UTF-8 produces fewer codepoints than raw bytes, the source
  // was multibyte UTF-8. Otherwise treat as Latin-1 (the fast path).
