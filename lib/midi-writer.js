...
// Push `arr` onto this.buffer in-place. Previously this was
// `this.buffer = this.buffer.concat(arr)`, which reallocates the entire buffer
// on every call — O(n^2) total when writeVarInt and writeString funnel small
// chunks through writeBytes once per event.
Writer.prototype.writeBytes = function(arr) {
  var buf = this.buffer
  var len = arr.length
  for (var i = 0; i < len; i++) buf.push(arr[i] & 0xFF)
}

// UTF-8 encode the string and push the bytes. Previously
...
Writer.prototype.writeString = function(str) {
  ...
  for (var i = 0; i < len; i++) buf.push(bytes[i])
}
...
// Inline varint encoding for the common 1–4 byte cases. Previously this
// funneled through writeBytes on a reversed temp array, inheriting the
// writeBytes concat cost once per event (charts have O(10k) events).
Writer.prototype.writeVarInt = function(v) {
  ...
  if (v <= 0x7F) {
    ...
    buf.push(((v >> 7) & 0x7F) | 0x80)
    buf.push(v & 0x7F)
  } else if (v <= 0x1FFFFF) {
    buf.push(((v >> 14) & 0x7F) | 0x80)
    buf.push(((v >> 7) & 0x7F) | 0x80)
    buf.push(v & 0x7F)
  } else if (v <= 0xFFFFFFF) {
    buf.push(((v >> 21) & 0x7F) | 0x80)
    buf.push(((v >> 14) & 0x7F) | 0x80)
    buf.push(((v >> 7) & 0x7F) | 0x80)
    buf.push(v & 0x7F)
  } else {
    ...
    for (var j = bytes.length - 1; j >= 0; j--) buf.push(bytes[j])
  }
}
